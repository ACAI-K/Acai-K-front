    ...
    return (
        <div class="bg-lucy-dark text-lucy-light flex flex-col justify-between font-work h-dvh w-dvw overflow-hidden mx-auto">
            ...
            <div class="grow w-full flex items-center justify-center p-6 h-auto">
                ...
                {/* INICIAR SESION */}
                <div class="w-full max-w-4xl rounded-4xl overflow-hidden shadow-2xl grid grid-cols-1 md:grid-cols-2 min-h-[600px]">
                    <CreateAcc />
                    <Show when={dorm && result()}>
                        <div class="flex flex-col justify-start p-8 w-[50dvw]] md:w-full gap-8">
                            <div class="flex w-[30dvw] items-start">
                                <img src={dorm.images[0]} alt={dorm.name} class="w-full h-full object-cover" />
                            </div>
                            <div class="flex flex-col justify-center p-8 w-full gap-8">
                            ...
                            <div class="h-2 bg-lucy-light w-full mt-4 mb-2"></div>
                            <div class="flex flex-between items-center">
                                <p class="text-3xl font-fira font-bold text-lucy-light">Total</p>
                                <p class="text-lg text-lucy-accent font-work">${habitacion?.precioNoche && habitacion.precioNoche * nochesEstaticas} MNX</p>
                            </div>
                            <LucyButton ref={"/pago"} ButtonBackground="lucy-primary" ButtonForeground="lucy-dark" ButtonText="Proceder al Pago" ButtonSize="md" ButtonIconSide="right" ButtonIcon={<ArrowLeft />} class="self-end mt-4 cursor-pointer" />
                        </div>
                    </Show>
                </div>
            </div>
